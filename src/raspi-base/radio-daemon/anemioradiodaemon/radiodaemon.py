...
def connect_db(db_name=DEFAULT_DB_NAME, logger=logging.getLogger()):
    ...
    db_conn = sqlite3.connect(db_name, uri=False)
    logger.info('Done.')
    return db_conn
...
class RadioDaemon():
    ...
    def _parse_messages(self, messages):
        ...
        for message in messages:
            ...
            if '[' in data:
                ...
                timestamps_lookup.append(
                    {
                        'start': data_part_start_index, 'end': data_part_end_index,
        ...
        if len(data) < 1:
            ...
            return None
        parsed = []
        breaks = [m.span() for m in re.finditer('\[[0-9]+\]', data)]  # noqa: W605
        if len(breaks) < 1:
        ...
        for i in range(0, len(breaks)):
            ...
            received_timestamp = next(
                x['received_timestamp'] for x in timestamps_lookup if breaks[i][0] >= x['start'] and breaks[i][0] <= x['end']
            )
            parsed.append(
                {
                    'radio_command': RadioCommands(int(radio_command)),
                    'contents': contents, 'received_timestamp': received_timestamp
                }
            )
                ...
                # Setup has finished, all devices should be reported as setup successful or unsuccessful.
                    ...
                    if m is not None:
                        ...
                        if actual_num_online != captured_num_online:
                            self.logger.error(
                                'Total online devices (%d) does not match total from previous messages (%d).',
                                actual_num_online,
                                captured_num_online
                            )
                        ...
                        if actual_num_offline != captured_num_offline:
                            self.logger.error(
                                'Total offline devices (%d) does not match total from previous messages (%d).',
                                actual_num_offline,
                                captured_num_offline
                            )
                ...
                if station_state in [
                    ...
                    for packet in radio.get_packets():
                        ...
                        messages_collected.append(
                            {
                                'data': message,
                                'received_timestamp': packet.received
                            }
                        )
    ...
    def record_location(self):
        ...
        if latlng is not None:
            with closing(self.db_conn.cursor()) as c:
                c.execute(
                    'INSERT INTO station_location(timestamp, lat, lon, is_actual) VALUES(?,?,?,?)',
                    (
